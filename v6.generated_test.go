...
func TestRedisProcessor_JsonClear(t *testing.T) {
	...
	// Clear the array
	clearRes, err := client.JsonClear(key, "numbersArray").Result()
	if assert.NoError(t, err) {
		assert.Equal(t, int64(1), clearRes)
	}

	// Verify array is now empty
	typeRes, err := client.JsonType(key, "numbersArray").Result()
	if assert.NoError(t, err) {
...
func TestRedisProcessor_JsonForget(t *testing.T) {
	...
	forgetRes, err := client.JsonForget(key, "keyA").Result()
	if assert.NoError(t, err) {
...
func TestRedisProcessor_JsonMerge(t *testing.T) {
	...
	// Merge a new field
	mergeRes, err := client.JsonMerge(key, ".", `{"newKey": "newValue"}`).Result()
	if assert.NoError(t, err) {
		assert.Equal(t, "OK", mergeRes)
	}

	// Verify the merge worked
	getRes, err := client.JsonGet(key, "newKey").Result()
	if assert.NoError(t, err) {
...
func TestRedisProcessor_JsonMSet(t *testing.T) {
	...
	defer client.Del(key1, key2)

	// Set multiple keys at once
	msetRes, err := client.JsonMSet(key1, ".", `{"a":1}`, key2, ".", `{"b":2}`).Result()
	if assert.NoError(t, err) {
		assert.Equal(t, "OK", msetRes)
	}

	// Verify both keys were set
	get1, err := client.JsonGet(key1).Result()
	if assert.NoError(t, err) {
		assert.Contains(t, get1, `"a"`)
	}

	get2, err := client.JsonGet(key2).Result()
	if assert.NoError(t, err) {
...
func TestRedisProcessor_JsonSetMode(t *testing.T) {
	...
	setRes, err := client.JsonSetMode(key, ".", `{"test":1}`, "NX").Result()
	if assert.NoError(t, err) {
		assert.Equal(t, "OK", setRes)
	}
	...
	setRes3, err := client.JsonSetMode(key, ".test", "99", "XX").Result()
	if err == nil {
...
func TestRedisProcessor_JsonToggle(t *testing.T) {
	...
	getRes, err := client.JsonGet(key, ".flag").Result()
	if assert.NoError(t, err) {
